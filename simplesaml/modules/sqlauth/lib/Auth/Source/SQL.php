...
class sspmod_sqlauth_Auth_Source_SQL extends sspmod_core_Auth_UserPassBase {
    ...
    private function connect() {
        try {
            ...
            throw new Exception('sqlauth:'.$this->authId.': - Failed to connect to \''.$this->dsn.'\': '.$e->getMessage());
        }
    ...
    protected function login($username, $password) {
        //$username = 'Learouma';
       	//$password = 'Kyar6K8';
        //error_log(__LINE__.' simplesaml SQL username defined  '.$username.' - '.$password);
        define('_JEXEC', 1);
        
        //define('JPATH_BASE', 'C:\wamp\www\extranet-epfc\\');
        define('JPATH_BASE', '/homez.313/epfckkdq/www/_dev-extranet');
        //define('JPATH_BASE', '/homez.313/epfckkdq/www/');
        define('DS', DIRECTORY_SEPARATOR);
        //old place for requirement
        ...
        try {
            ...
            //modification bertrand
            //$res = $sth->execute(array($username, $password));
        ...
        SimpleSAML_Logger::info('sqlauth:'.$this->authId.': Got '.count($data).' rows from database');
        //crypt password with Joomla
        $credentials['admin'] = $username;
        $credentials['password'] = $password;
        $row->password = $data['0']['password'];
        
        $parts = explode(':', $row->password);
        $crypt = $parts[0];
        $salt = @$parts[1];
		/* Required Joomla Files */
		require_once(JPATH_BASE.DS.'libraries'.DS.'simplesamlphp'.DS.'linkJoomla.php');
		
		$token = linkJoomla::getToken();
        //$testcrypt = JUserHelper::getCryptedPassword($credentials['password'], $salt);
		$testcrypt =  linkJoomla::testcrypt($credentials['password'], $salt);
		/*echo $crypt.'<br />';
		echo $testcrypt.'<br />';
		die();*/
		if ($crypt != $testcrypt) {
            /* No rows returned - invalid username/password. */
            SimpleSAML_Logger::error('sqlauth:'.$this->authId.': No rows in result set. Probably wrong username/password.');
        ...
        //SimpleSAML_Logger::info('sqlauth:'.$this->authId.': Attributes: '.implode(',', array_keys($attributes)));
        
        //be modification du sql auth
        //$token = 'wawawou';
        $attributes['token'][] = $token;
        //$log =  '<pre>'.print_r($attributes, true).'</pre>';
        //file_put_contents  ('/homez.313/epfckkdq/www/_dev-extranet/log-plugin.txt',$log, FILE_APPEND);
        return $attributes;
    }
