 ...
 * @version $Id: Session.php 1826 2009-10-07 09:11:48Z olavmrk $
 */
class SimpleSAML_Session {
	...
	private function __construct() {
		...
		//error_log(__LINE__.' simple saml session __contruct je donne un trackid');
		//$this->trackid = '4156206af2' ;
		
		//error_log(__LINE__.' trackid simplesaml '.$this->trackid);
	...
	public static function getInstance() {
		//echo 'session <pre>'.print_r(unserialize($_SESSION['SimpleSAMLphp_SESSION']),true).'</pre>';
		//die();

		/* Check if we already have initialized the session. */
		if (isset(self::$instance)) {
			//echo __LINE__.' Check if we already have initialized the session</br>';
		...
		if(self::$instance !== NULL) {
			//echo __LINE__.' Check if we have stored a session stored with the session</br>';
	...
	private static function loadSession() {
		...
		$sh = SimpleSAML_SessionHandler::getSessionHandler();
		//$beSession = unserialize($_SESSION['SimpleSAMLphp_SESSION']);
		//echo '<pre>'.print_r($beSession, true).'</pre>';
