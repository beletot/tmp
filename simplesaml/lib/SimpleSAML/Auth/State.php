...
class SimpleSAML_Auth_State {
	...
	public static function loadState($id, $stage) {
		//echo $_SERVER[__FILE__];
		//print_r(__FILE__);
		//joomla me fait perdre le state
		if($id == NULL){
			echo '<h1 style="color:red;">ERROR : id = null</h1>';
		}else{
			//echo '<h4 style="color:green;">OKAY : id => '.$id.'</h4>';
		}
		if(!$stage){
			//echo '<h1 style="color:red;">ERROR : $stage = null</h1>';
		}
		//echo '$authStateId #  <h4>'.$id.'</h4>';
		//echo 'sspmod_core_Auth_UserPassBase::STAGEID # <h4>'.$stage.'</h4>';
		assert('is_string($id)');
		assert('is_string($stage)');
